...
export default function draftprojects() {
    ...
    const [searchQ, setSearchQ] = useState('');
      ...
      const {alldata, loading} = useFetchData('/api/projects',refresh);
  ...
  const router = useRouter();
      ...
      function goBack() {
       // console.log("BlogAllData", BlogAllData)
         setRefresh(!refresh);
         setDelete(false);
        router.push('/projects/process');
         
     }
     async function deletBlog(id) {
        // console.log("delete id", id)
         await axios.post("/api/projects", {projectId: id}).then(res => {
    ...
    const publishedBlog = curreBlogs.filter(ab => ab.status === 'draft' )
    const pageNum = [];
  ...
  return  <LoginLayout>
    <> 
                  <div className="blogpage">
                           ...
                           <div>
                                <h2>All Drafted <span>Projects</span></h2>
                                <h3>ADMIN PANEL</h3>
                           </div>
                  ...
                  <div className="blogstable">
                     <div className="flex gap-2 mb-1">
                         ...
                         <input type="text" onChange={(e) => setSearchQ(e.target.value)} placeholder="search by title..." />
                     </div>
                          ...
                          <tbody>
                            {
                                ...
                                :<>
                                {
                                    ...
                                    :
                                (
                                    publishedBlog.map((blogs, index) =>{
                                        return (
                                            <tr key={blogs._id}>
                                               <td>
                                                 {indexfirstString + index + 1}
                                               </td>
                                               <td><img src={blogs.images[0]} width={180} alt="image"/></td>
                                              <td><h3>{blogs.title}</h3></td>
                                              <td><h3>{blogs.client}</h3>
                                              </td>
                                              <td><p style={{fontSize: 18}}>{blogs.description.slice(0, 100)}...  </p></td>
                                               <td>
                                                  <div className="flex gap-2 flex-center">
                                                        <Link href={`/projects/edit/${blogs._id}`} >
                                                        <button><FaEdit /></button>
                                                        </Link>
                                                        <button onClick={() => chooseDel(blogs._id)}><RiDeleteBin6Fill/></button>
