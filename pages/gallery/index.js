...
export default function gallery() {
    ...
    const [searchQ, setSearchQ] = useState('');
      ...
      const {alldata, loading} = useFetchData('/api/photos',refresh);
  ...
  const router = useRouter();
      ...
      async function chooseDel(id) {
         axios.get('/api/photos?id=' + id).then(res => {
      ...
      //  console.log("BlogAllData", BlogAllData)
         setRefresh(!refresh);
         setDelete(false);
        router.push('/gallery/process1');
         
     }
     async function deletBlog(id) {
        // console.log("delete id", id)
         await axios.post("/api/photos", {projectId: id}).then(res => {
                          ...
                          <tbody>
                            {
                                ...
                                :<>
                                {
                                    ...
                                    (
                                        <tr>
                                            <td colSpan={6} className="text-center" >No Photos Found</td>
                                        </tr>
                                    )
                                    :
                                (
                                            ...
                                            <tr key={blogs._id}>
                                              ...
                                              <td>
                                                  <div className="flex gap-2 flex-center">
                                                        <Link href={`/gallery/edit/${blogs._id}`} >
